...
int main(int argc, char *argv[])
{
   ...
   #pragma omp parallel private(i, x, tid) shared(area_par) num_threads(num_threads)
   {
      tid = omp_get_thread_num();
      x = ((b-a)/num_threads)*tid;
      
      #pragma omp for reduction(+:area_par)
      for(i = 0; i < N; i++) {
